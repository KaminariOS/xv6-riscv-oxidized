...
pub fn ringbuf_finish_read(ringbuf_desc: usize, bytes: usize) {
    USER_RING_BUFS
        .lock()
        .ringbuf_finish_read(ringbuf_desc, bytes)
}
...
pub fn ringbuf_finish_write(ringbuf_desc: usize, bytes: usize) {
    USER_RING_BUFS
        .lock()
        .ringbuf_finish_write(ringbuf_desc, bytes)
}
    ...
    pub fn ringbuf_start_read(&mut self, ringbuf_desc: usize) -> (usize, usize) {
        if let Some(user_ring_buf) = &mut self.0[ringbuf_desc] {
            ...
            if !book_ptr.is_null() {
                ...
                let res = (
                    book.get_diff(),
                    user_ring_buf.buf + (book.n_reads % (PAGE_SIZE * RINGBUF_SIZE)),
                );
                book.unlock();
                return res;
            } else {
    ...
    pub fn ringbuf_start_write(&mut self, ringbuf_desc: usize) -> (usize, usize) {
        if let Some(user_ring_buf) = &mut self.0[ringbuf_desc] {
            ...
            if !book_ptr.is_null() {
                ...
                book.lock();
                let res = (
                    get_ringbuf_buf_size() - (book.n_writes - book.n_reads),
                    user_ring_buf.buf + (book.n_writes % get_ringbuf_buf_size()),
                );
                book.unlock();
                return res;
            } else {
